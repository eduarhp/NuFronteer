	...
	foreach ($options as $value) {
		...
		case ($value['type'] == 'font-select'):
			...
			foreach ($value['options'] as $key => $option ) {
				$selected = '';
				 ...
				 $output .= '<option'. $selected .' value="' . esc_attr( $key ) . '">' . esc_html( $option['name'] ) . '</option>';
			 } 
			 $output .= '</select>';
		break;
		...
		case 'typography':	
			...
			foreach ( $faces as $key => $face ) {
				$output .= '<option style="font-family: ' . esc_html( $face ) . ', sans-serif;" value="' . esc_attr( $key ) . '" ' . selected( $typography_stored['face'], $key, false ) . '>' . esc_html( $face ) . '</option>';
			}			
